...
$$
E[\hat{\beta_1}] = \beta_1 + \beta_2 \frac{\text{Cov}(x_1, x_2)}{\text{Var}(x_1)}
$$
...
Providers do not charge a price premium for unsafe sex with clients who are are regulars.
A potential reason could be that regulars are perceived to be less risky in terms of carrying STIs. 
...
```
```{block, solution = TRUE}
If we want to take the point estimates seriously (ignoring the std errors - purely for the sake of interpretation practice), we'd see that there seems to be evidence of price discrimination --but in the other direction.
